...
// Assembly translation
	...
	// Prune if not on screen
	#ifdef DOUBLE_WIDTH
	/*
		// if (FANTY_ENX < scroll_x) return;
		__asm__ ("lda %v", FANTY_ENX);
		__asm__ ("cmp %v", scroll_x);
		__asm__ ("lda %v+1", FANTY_ENX);
		__asm__ ("sbc %v+1", scroll_x);
		__asm__ ("bcs %g", prune_check_2);
		__asm__ ("jmp %g", ef_done);

		// if (scroll_x + 240 < FANTY_ENX) return;
	prune_check_2:
		__asm__ ("lda %v", scroll_x_r);
		__asm__ ("cmp %v", FANTY_ENX);
		__asm__ ("lda %v+1", scroll_x_r);
		__asm__ ("sbc %v+1", FANTY_ENX);
		__asm__ ("bcs %g", prune_check_done);
		__asm__ ("jmp %g", ef_done);
	*/
		__asm__ ("lda %v", on_screen);
		__asm__ ("bne %g", prune_check_done);
		__asm__ ("jmp %g", ef_done);
	...
	FANTY_RDX = FANTY_ENX;	// cc65 resolves this just fine.
		...
		// If the increment for this frame is zero,
		// just do NOTHING!
		__asm__ ("lda %v, y", fanty_incs);
		__asm__ ("beq %g", ef_mx_collide_done);
