' mkts v0.8.2 20190611 - Converts several kinds of tilesets
...
Dim Shared As Integer blackiszero, columns, nextPattern, mirrored, simplemeta, supersimplemeta, aseprite, noskipempty, allbg, deinterlaced, debug, precalc_attrs
...
Sub nesDoTexts (img As Any Ptr, pal () As Integer, xOrg As Integer, yOrg As Integer, wProc As Integer, hProc As Integer, tsMapFn As String, label As String, offset As Integer, max As Integer, fOut As Integer)
	...
	If fOut = -1 And Not ignoreMapFile Then
		...
		Print #fOut, "// Copyleft 2015 - 2019 The Mojon Twins"
		Print #fOut, " "
	End If
...
Sub nesDoTmaps (img As Any Ptr, pal () As Integer, xOrg As Integer, yOrg As Integer, wProc As Integer, hProc As Integer, wMeta As Integer, hMeta As Integer, tsMapFn As String, label As String, offset As Integer, max As Integer, fOut As Integer)
	...
	If precalc_attrs Then deinterlaced = -1
	...
	If Not ignoreMapFile Then
		If deinterlaced Then
			...
			If precalc_attrs Then
				For i = 0 To 6 Step 2
					Print #fOut, "const unsigned char " & label & "_pals_" & (i\2) & " [] = {"
					ctr = 0
					For j = 0 to tMapsIndex - 1
						If ctr = 0 Then Print #fOut, "	";
						Print #fOut, "0x" & lcase (Hex (byteArrays (16, j) Shl i, 2));
						If j < tMapsIndex - 1 Then Print #fOut, ", ";
						ctr = ctr + 1: If ctr = 16 Then ctr = 0: Print #fOut, ""
					Next j
					If ctr Then Print #fOut, ""
			...
			For yy = 1 To hMeta
				...
				For xx = 1 To wMeta
					...
					If metapals (tMapI) = &Hff then
						Print #fOut, "                        ";
					Else
						Print #fOut, "0x" & Lcase (Hex (ya, 2)) & ", 0x" & Lcase (Hex (xa, 2)) & ", 0x" & Lcase (Hex (tMap (tMapI), 2)) & ", 0x" & Lcase (Hex (metapals (tMapI), 2));
						Print #fOut, ", ";
					End If
					tMapI = tMapI + 1
					xa = xa + 8
				...
				For yy = 1 To hMeta
					...
					For xx = 1 To wMeta
						...
						If metapals (tMapI) = &Hff then
							Print #fOut, "                        ";
						Else
							Print #fOut, "0x" & Lcase (Hex (ya, 2)) & ", 0x" & Lcase (Hex (xa, 2)) & ", 0x" & Lcase (Hex (tMap (tMapI), 2)) & ", 0x" & Lcase (Hex (64 Xor metapals (tMapI), 2));
							Print #fOut, ", ";
						End If 							
						tMapI = tMapI + 1
						xa = xa + 8
				...
				For yy = 1 To hMeta
					...
					For xx = 1 To wMeta
						...
						If metapals (tMapI) = &Hff then
							Print #fOut, "                        ";
						Else
							Print #fOut, "0x" & Lcase (Hex (ya, 2)) & ", 0x" & Lcase (Hex (xa, 2)) & ", 0x" & Lcase (Hex (tMap (tMapI), 2)) & ", 0x" & Lcase (Hex (128 Xor metapals (tMapI), 2));
							Print #fOut, ", ";
						End If
						tMapI = tMapI + 1
						xa = xa + 8
	...
	While Not Eof (fIn)
		...
		Select Case cmd
			...
			Case "MAPFILE":
				...
				Print #fMapFile, "// Copyleft 2015 - 2019 The Mojon Twins"
				Print #fMapFile, ""
			Case "IGNOREMAPFILE":
				ignoreMapFile = -1
			Case "USEMAPFILE":
			...
			Case "MAPPED"
				...
				precalc_attrs = parserFindTokenInTokens ("PRECALC_ATTRS", tokens (), "ucase")
...
filteredPuts "mkts v0.8.2 20190611"
filteredPuts "Cuts & imports tilesets from a png."
filteredPuts "Copyleft 2015 - 2019 The Mojon Twins"
filteredPuts ""
...
Select Case platform
	...
	Case PLATFORM_GB:
		...
		Print #fOut, "// Copyleft 2017, 2019 The Mojon Twins"
		Print #fOut, ""
		...
		Print #fOut, ""
		Print #fOut, "};"
		Print #fOut, "#define " & uCase (sclpGetValue ("label")) &"_PATTERNS " & mainIdx\16

		' Calculate palette byte

		palByte = 0
		For i = 0 To 3
			palByte = palByte Or (pal (i) SHL i*2 )
		Next i
		Print #fOut, "#define " & uCase (sclpGetValue ("label")) &"_PALETTE 0x" & Hex (palByte, 2)
		Print #fOut, ""
		Close #fOut
