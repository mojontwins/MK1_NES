...
Dim Shared As Integer retValue
...
'	Puts("enter if platform <> SG1000 ")
	
	img = imageread_png (sclpGetValue ("path"),sclpGetValue ("pals"),32)
...
img = imageread_png (sclpGetValue ("path"),sclpGetValue ("in"),32)
If ImageInfo (img, w, h, , , , ) Then
	' Error!
End If
...
If platform = PLATFORM_NES Then
	Select Case sclpGetValue ("mode")
		...
		Case "pals":
'			Puts ("Execute doPals")
'			Puts ("Value pals:"+sclpGetValue ("pals"))
