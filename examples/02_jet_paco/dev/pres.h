...
void __fastcall__ game_ending (void) {
	...
	pr_str (3, 14, "ALL CRISTALS ARE RECOVERED");
	pr_str (2, 16, "NOW YOU CAN GET BACK TO YOUR");
	pr_str (3, 18, "BASE AND HAVE A GOOD REST!");		
	common_pres_b ();
}
...
unsigned char title (void) {
	...
	for (rdy = 21; rdy < 27; rdy ++) pr_str (10, rdy, "            ");
	pr_str (10, 21, "PRESS SELECT");
	pr_str (10, 23, "  MISSION A ");
	pr_str (10, 25, "  MISSION B ");
	ppu_on_all ();
	fade_delay = 4;
	fade_in ();
	...
	while (1) {
		oam_meta_spr (80, 164 + (gpit << 4) + SPRITE_ADJUST, 128, spr_pl1_1);
		ppu_waitnmi ();
		i = pad_poll (0);
		...
		if (i & PAD_SELECT) {
			sfx_play (0, 1); 
			//gpit = 1 - gpit; 
			if (gpit) gpit = 0; else gpit = 1;
			while (pad_poll (0) & PAD_SELECT);
		}
		if (i & PAD_UP) {
			sfx_play (0, 1);
			if (gpit) gpit --; else gpit = 2;
			while (pad_poll (0) & PAD_UP);
		}
		if (i & PAD_DOWN) {
			sfx_play (0, 1);
			if (gpit < 2) gpit ++; else gpit = 0;
			while (pad_poll (0) & PAD_DOWN);
		}
	}
...
void __fastcall__ credits (void) {
	...
	pr_str (5, 22, "JET PACO - SPACE AGENT!");
