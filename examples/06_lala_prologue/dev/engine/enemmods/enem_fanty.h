...
if (enf_vx [gpit]) {
	...
	if (enf_vx [gpit] > 0) {
		ptx2 = (en_x [gpit] + 11) >> 4;
		if (FATTR (ptx2, pty1) || FATTR (ptx2, pty2)) {
		...
		ptx1 = (en_x [gpit] + 4) >> 4;
		if (FATTR (ptx1, pty1) || FATTR (ptx1, pty2)) {
			enf_vx [gpit] = -enf_vx [gpit];
...
if (enf_vy [gpit]) {
	...
	if (enf_vy [gpit] > 0) {	
		pty2 = (en_y [gpit] + 15) >> 4;
		if (FATTR (ptx1, pty2) || FATTR (ptx2, pty2)) {
		...
		pty1 = (en_y [gpit] + 8) >> 4;
		if (FATTR (ptx1, pty1) || FATTR (ptx2, pty1)) {
			enf_vy [gpit] = -enf_vy [gpit];
