...
void open_box (unsigned char what) {
	ppu_waitnmi ();

	// Wait for button
	if (what) {
		while (pad_poll (0) & (PAD_A | PAD_B)) ppu_waitnmi ();
	...
	rdy = 3; while (rdy < 9) {
		rdx = 4; while (rdx < 12) {
			...
			update_list_tile (rdx + rdx, TOP_ADJUST + rdy + rdy, what ? map_buff [rdx + (rdy << 4)] : stbf [gpit ++]); 
			rdx ++;
			if ((rdx & 3) == 0) ppu_waitnmi ();
		}
		rdy ++;
	}
	stbl = TOP_ADJUST + 7;
	...
	clear_update_list ();
	update_index = 0;
	if (what) while (pad_poll (0) & (PAD_A | PAD_B)) ppu_waitnmi ();
}
#endif
 
void p_s (unsigned char x, unsigned char y, unsigned char *s) {
	vram_adr (0x2000 + (y << 5) + x);
